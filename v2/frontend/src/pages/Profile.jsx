...
//         // If we have user data from auth context, use that email instead of test email
//         if (user && user.email) {
//           profileData.email = user.email;
//         }
        
//         // If we have user data from auth context, use that name if the profile has the default name
//         if (user && user.name && (profileData.name === "Test User" || !profileData.name)) {
...
const Profile = () => {
  ...
  useEffect(() => {
    const loadProfile = async () => {
      try {
        ...
        // Use the real email from auth context instead of the test email
        if (user && user.email) {
          profileData.email = user.email;
        }
        ...
        try {
          ...
          console.error("Error loading API keys:", keyError);
          // Don't fail the whole profile load if API keys fail
          setApiKeys([]);
        }
      } catch (error) {
        console.error("Error loading profile:", error);
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      // Call the updateUserProfile API with proper data structure
        ...
        <Grid item xs={12}>
          ...
          >
            <Avatar
              ...
              sx={{ width: 120, height: 120, bgcolor: "primary.main" }}
            >
              {!avatarUrl && (profile?.name?.[0] || <PersonIcon fontSize="large" />)}
            </Avatar>
            ...
            <Box sx={{ flex: 1 }}>
              ...
              <Typography variant="body1" color="text.secondary" gutterBottom>
                {/* Always display the real email from auth context if available */}
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 1, mt: 1 }}>
                ...
                />
                <Chip 
                  label={`Member since ${new Date(profile?.createdAt || new Date()).toLocaleDateString()}`} 
                  variant="outlined" 
                />
              </Box>
            </Box>
                ...
                {apiKeys.length === 0 ? (
                  ...
                  apiKeys.map((key) => (
                    <ListItem key={key.id} divider>
                      ...
                      <ListItemText
                        ...
                        secondary={
                          <Box>
                            ...
                            <Typography variant="caption" display="block">
                              Last used: {new Date(key.lastUsed || new Date()).toLocaleDateString()}
                            </Typography>
                          </Box>
                        }
                      />
                      <ListItemSecondaryAction>
                        {!key.isDefault && (
                          <IconButton edge="end" onClick={() => handleSetDefaultKey(key.id)} title="Set as default">
                            <CheckIcon />
                          </IconButton>
                        )}
                        <IconButton edge="end" onClick={() => handleDeleteKey(key.id)} title="Delete key">
                          <DeleteIcon />
                        </IconButton>
                      </ListItemSecondaryAction>
                    </ListItem>
